...
/*                                                                         */
/*                                                                         */
/*  Modification   1.2-006  , sis3153 VME D16/D8 cycles                    */
/*              marked with "modification 2015-03-16"                      */
/*                                                                         */
/*-------------------------------------------------------------------------*/
/*                                                                         */
...
static const int    productIdUsb3 = 0x3153;          /* Product ID we are also looking for */
			   ...
			   unsigned int                     max_usb_device_Number)
{
  ...
  while (busses) {
    ...
    while (aDevice) {
	  ...
	  ( (aDevice->descriptor.idProduct == productId) || (aDevice->descriptor.idProduct == productIdUsb3) )) {
	
	/* Got an SIS 315x device: */
...
*/
int
Sis3150usb_OpenDriver_And_Download_FX2_Setup(PCHAR usbDeviceName, struct SIS3150USB_Device_Struct* sis3150usb_Device)
{
  ...
  if(sis3150usb_Device->idProduct == productId){
	    ...
	    strncat(usbDeviceFile, sis3150usb_Device->cDName, PATH_MAX);

	    /* See if the USB special file exists.                   */

	    
	    hardwareHandle = getDeviceHandle(sis3150usb_Device->cDName);
	    if (!hardwareHandle) {
	      return -ENODEV;
	    }
	...
	/*  begin modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */
		...
		sis3153_valid_flag = TRUE;
	    }
	/*  end modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */

  /*  Return a new open device for the user to use.           */
  
  return Sis3150usb_OpenDriver(usbDeviceName, sis3150usb_Device);
}
/*!
...
/*  begin modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */
  unsigned int protocol_req_nof_bytes ;
  protocol_req_nof_bytes = req_nof_bytes ;
/*  end modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */
...
/*  begin modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */
	if(sis3153_valid_flag == FALSE){
	  ...
	  data_copy_nof_words  =  (req_nof_bytes>>2 ) ; 
 	  switch (vme_size) {
		case 1:	   // 1 Byte
			data_copy_nof_words  =  1 ; 
 			protocol_req_nof_bytes = 2 ;
	   		cSize = 0x0 ; //  1 Byte
			break;
		case 2:	   // 2 Bytes
			data_copy_nof_words  =  1 ; 
...
/*  begin modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */
	cUsbBuf[4]  =   (char)  protocol_req_nof_bytes  ;       //length 7:0
	cUsbBuf[5]  =   (char) (protocol_req_nof_bytes >> 8);    //length 15:8
/*  end modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */
	cUsbBuf[6]  =   (char)  vme_am ;   
	cUsbBuf[7]  =   (char) (vme_am >> 8);
